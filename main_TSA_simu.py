...
def parse():
    ...
    parser.add_argument('--dataset_dir', default="./datasets/TSA_simu/")
...
def load_data(path, scene, T_CA):
    ...
    hsi = torch.from_numpy(foo['img']).float().to(args.device)
...
if __name__ == "__main__":
    ...
    for i in range(nSamples):
        ...
        preds[i] = X_star.cpu()
        truths[i] = truth.cpu()
    scio.savemat("./results/recon_TSA_simu.mat", {"pred":preds, "truth":truths})