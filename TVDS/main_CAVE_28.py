...
def load_data(path, scene, T_CA):
    ...
    truth = torch.from_numpy(foo['cave_data']).float().to(args.device)
    # sRGB to linear
    rgb = torch.from_numpy(foo['cave_rgb']).float().to(args.device) ** 2.2
...
if __name__ == "__main__":
    ...
    for i in range(nSamples):
        ...
        X_star = pipeline(rgb, cassi, coded_aperture_mask, args).clamp(0, 1)
        toc = time.time()
    ...
    scio.savemat(save_path/"recon_CAVE.mat", {"preds":preds, "truths":truths, "Metrics":Metrics})
