...
def calculate_psnr_with_quantization(
...
def calculate_sam_with_quantization(
    ground_truth: torch.Tensor, 
    predicted: torch.Tensor,
    value_range: int = MAX_VAL_12_BIT, # MST's code corresponding to set value_range = 255 (8 bit)
) -> torch.Tensor:
...
def calculate_ssim_with_quantization(
    ground_truth: torch.Tensor, 
    predicted: torch.Tensor,
    value_range: int = MAX_VAL_12_BIT, # MST's code corresponding to set value_range = 255 (8 bit)
) -> torch.Tensor:
    ...
    Returns:
        Mean SAM value in degrees
    """
    ground_truth = (ground_truth.clip(0, 1) * value_range).round().cpu().numpy()
    predicted = (predicted.clip(0, 1) * value_range).round().cpu().numpy()

    ssim_value = ssim(ground_truth, predicted, data_range=value_range, multichannel=True)# type: ignore
